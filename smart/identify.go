...
func Identify(dev *os.File) (IdentifyDeviceData, error) {
	...
	if err := SendCDB(dev, cdb16[:], &respBuf); err != nil {
		return identBuf, fmt.Errorf("SCSI Passthrough ATA IDENTIFY: %v", err)
	}

	binary.Read(bytes.NewBuffer(respBuf), native_endian.NativeEndian(), &identBuf)
